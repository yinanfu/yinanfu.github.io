...
vector<vector<int> > allPartMeeting(vector<string> inputName)
{
    ...
    if(iter!=inputName.end()){
        ...
        // if(mapA.count(*lastIter)!=0)
        tmpRes=twoPartMeeting(mapA[*lastIter],mapA[*iter]);
        // else{
        //     //error
        // }
        iter++;
...
int main(int argc, char const *argv[])
{
    ...
    struct addrinfo *servinfo, *scheduler;
    int s;
    // int sockfd;
    ...
    if ((status = getaddrinfo(IP_ADDR, SERVER_A_PORT, &hints, &servinfo)) != 0)
    {
        fprintf(stderr, "SERVER A set up incorrectly: %s\n", gai_strerror(status));
        exit(1);
    }
    ...
    fprintf(stdout, "SERVER A is up and running using UDP on port %s.\n", SERVER_A_PORT);
    // fprintf(stdout, "SERVER A has total capactity %d and initial occupancy %d\n", total_cap, cur_occup);
    ...
    fprintf(stderr, "Server A finished sending a list of usernames to Main Server.\n");
    ...
    // vector<vector<int> > test1=twoPartMeeting(mapA["khloe"],mapA["eli"]);
